...
Defined in `data-pipeline/atalayalab/io/contract.py`. A parameter row is **accepted** iff it satisfies the schema;
...
Each pipeline run writes a compact trace (`derived/<case>/trace.json`, schema `atalaya.trace/v1`) and a manifest
(`derived/manifests/<case>.json`, schema `atalaya.manifest/v2`) recording params, seed, engine+version, the
...
redistribution terms (only the public derived artifacts are committed; raw/private sources stay out-of-git,
provided via the environment, never checked in).
