...
`data-pipeline/atalayalab/pipeline.py` orchestrates the **named stages** (frozen names/signatures, rework bodies):
...
Run: `python -m atalayalab.pipeline [all|<case_id>] [--seed N]` (or `scripts/precompute.{sh,ps1}`). It writes
