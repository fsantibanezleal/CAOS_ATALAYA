...
`data-pipeline/atalayalab/io/contract.py`. Declares the required schema (columns, units, ranges) + an explicit
...
`data-pipeline/atalayalab/core/{trace.py, manifest.py}`. Every run writes a compact trace (`atalaya.trace/v1`) +
a manifest (`atalaya.manifest/v2`) recording params, seed, engine+version, the artifact byte size, the measured
