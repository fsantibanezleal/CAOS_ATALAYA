...
2. **Rename** the package `atalayalab` → `<slug>lab` (the folder + all imports + `pyproject.toml`
   `[tool.setuptools.packages.find].where`/name + the scripts' `-m atalayalab.pipeline` + docs).
