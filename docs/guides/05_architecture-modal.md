...
Binding decision: ADR-0058 (the in-app architecture modal). Reference implementations: Veta and Circuita.
...
2. **Specialise** the product-specific tabs for Atalaya:
   - Replace `public/svg/tech/01-the-app.svg` with the Atalaya domain flow (Data Observatory catalog, harvest with
     the tier gate, profile, mine the five relation kinds, explore the graph) and edit the `app` tab's
     `body_en` / `body_es`.
   - Replace `public/svg/tech/04-the-science.svg` with Atalaya's real algorithm: the MiniLM embeddings, MinHash
     containment for joinability, Spearman plus permutation-null plus FDR for correlation, and the calibrated
     multi-evidence affinity (its equation is in [../architecture/06_model-evaluation.md](../architecture/06_model-evaluation.md)
     and [../cases/AFF_top.md](../cases/AFF_top.md)). Edit the `science` tab body.
   - Tabs `lanes`, `web-flow`, `design` are archetype-generic. For Atalaya, `lanes` should say the live lane is
     ONNX semantic search plus JS affinity reweight (not Pyodide), and `design` should name the two contracts plus
     the cases-by-category mechanism. Keep the shipped SVGs otherwise.
