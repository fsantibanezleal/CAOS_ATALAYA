...
const CART_map = (lang: Language) => block(lang,
  ...
  },
  {
    ...
    components: li(["Nodos: uno por dataset perfilado.", "Posición: proyección PCA 2-D del embedding multilingüe (un toggle 2D/3D también orbita un layout PCA 3-D).", "Color: la faceta elegida (tema, origen, clúster, claves, actualidad, tasa de nulos o subcategoría).", "Lectura: título, tema, editor, tamaño, claves, extensión temporal, fracción de nulos, y la lista completa de subcategorías OECD (temas) del dataset."]),
    ...
    variants: <p>Las siete variantes recolorean los mismos puntos: por <em>tema</em>, <em>origen</em> (curado vs DOI), <em>clúster</em> (k-means sobre embeddings), <em>claves de unión</em>, <em>actualidad</em> (viridis sobre el último año), <em>tasa de nulos</em>, y <em>subcategoría</em> (la taxonomía OECD completa de 27 valores, más fina que los cinco temas). El layout no cambia, así se comparan facetas sobre un mapa fijo; un toggle 2D/3D alterna entre el mapa plano SVG y una vista PCA 3-D orbitable.</p>,
    howto: <p>Buscar clústers de color compactos (un tema semánticamente coherente) y outliers (un dataset lejos de sus pares). Al pasar el cursor por un punto se lee el dataset; cambiar la faceta de color plantea otra pregunta sobre la misma geografía.</p>,
  });
...
const SEM_network = mk(
  { problem: <p>Which datasets describe the same kind of thing, purely from their text? This network links datasets whose embeddings are close.</p>,
    ...
    howto: <p>Raise the threshold to find the most confidently related pairs; hover a node to list its neighbours with their cosine. Use it to discover unexpected related datasets.</p> },
  { problem: <p>¿Qué datasets describen lo mismo, solo por su texto? Esta red une datasets con embeddings cercanos.</p>,
    ...
    howto: <p>Subir el umbral para hallar los pares más confiables; al pasar el cursor por un nodo se ven sus vecinos con su coseno.</p> });

const JOIN = mk(
  { problem: <p>Which datasets can actually be <em>joined</em>? Two datasets are joinable when they share an entity key whose values overlap, so they can be merged for analysis.</p>,
    ...
    variants: <p>The variants raise the containment threshold (0.5, 0.95). At 0.95 only near-perfect foreign-key relationships remain.</p>,
  ...
  { problem: <p>¿Qué datasets se pueden <em>unir</em> de verdad? Dos datasets son unibles cuando comparten una clave de entidad cuyos valores se solapan.</p>,
    ...
    howto: <p>Subir el umbral para hallar claves limpias; al pasar el cursor por un nodo se ve con qué datasets une y por qué columna.</p> });

const CORR = mk(
  { problem: <p>Do two datasets, aligned on a shared entity (comuna or region), actually <em>correlate</em>? These are cross-dataset relationships that survive a real significance test.</p>,
  ...
  { problem: <p>¿Dos datasets, alineados sobre una entidad común (comuna o región), realmente <em>correlacionan</em>? Son relaciones entre datasets que sobreviven a una prueba de significancia real.</p>,
    ...
    howto: <p>Ordenar por ρ o p ajustada; el diagrama de dispersión ubica cada hallazgo como ρ vs significancia. Al pasar el cursor se ven las columnas exactas y el n detrás del número.</p> });
...
const GEO = mk(
  ...
  { problem: <p>¿Qué tan anclado geográficamente está el catálogo? ¿Qué datasets se ubican en una comuna, región o punto, y dónde están los georreferenciados?</p>,
    ...
    howto: <p>Las barras dimensionan cuánto del catálogo es unible a un mapa administrativo chileno; al pasar el cursor por los puntos se ve qué datasets llevan coordenadas.</p> });
...
const TIME = mk(
  ...
  { problem: <p>¿Cuándo viven los datasets en el tiempo y dónde se solapan sus ventanas? El solape temporal es precondición de cualquier análisis longitudinal entre datasets.</p>,
    ...
    howto: <p>Los años de mayor cobertura (mejores para un cruce transversal) y los datasets de extensión larga (mejores para tendencias); al pasar el cursor por una barra se ve su ventana exacta.</p> });

const QC = mk(
  { problem: <p>How clean is the catalog, table by table? Before trusting any relation, the null rates, table shapes, key coverage and ingestion flags each table raised all matter.</p>,
  ...
  { problem: <p>¿Qué tan limpio está el catálogo, tabla por tabla? Antes de confiar en cualquier relación hay que conocer las tasas de nulos, las formas de tabla, la cobertura de claves y los flags que cada tabla levantó en ingesta.</p>,
    ...
    howto: <p>Ordenar la tabla para hallar los datasets más sucios; revisar las distribuciones de tipos/flags para entender problemas sistemáticos antes de unir o correlacionar.</p> });

const AFF = mk(
  { problem: <p>Given three different reasons two datasets might be related (they read alike, they join, they correlate), which pairs are related once the evidence is fused? This is Atalaya's novel proposal.</p>,
    components: li(["Rows: dataset pairs ranked by a fused affinity score.", "Stacked bar: the contribution of each evidence (semantic / joinability / correlation).", "Live sliders: the three evidences reweight and the ranking recomputes in the browser."]),
    ...
    variants: <p>The presets bias the weights (balanced, semantic-led, join-led, correlation-led) and limit to the top pairs or a minimum score; the live sliders allow any weighting.</p>,
  ...
  { problem: <p>Dadas tres razones distintas por las que dos datasets podrían relacionarse (se leen parecido, se unen, correlacionan), ¿qué pares están relacionados al fusionar la evidencia? Esta es la propuesta novel de Atalaya.</p>,
    components: li(["Filas: pares de datasets ordenados por un score de afinidad fusionado.", "Barra apilada: la contribución de cada evidencia (semántica / unión / correlación).", "Sliders en vivo: re-ponderan las tres evidencias y el ranking se recomputa en el navegador."]),
    ...
    howto: <p>Al mover los sliders se ve qué pares suben bajo cada criterio; la barra apilada explica <em>por qué</em> un par queda donde queda. Sirve para preseleccionar datasets que valga la pena unir.</p> });
