...
// data-pipeline/atalayalab/core/{trace.py, manifest.py}. A drift here makes `tsc` fail -> the contract is
...
export interface Trace {
  schema: string; // "atalaya.trace/v1"
  case_id: string;
  t: number[];
  S: number[];
  I: number[];
...
export interface CaseManifest {
  schema: string; // "atalaya.manifest/v2"
  case_id: string;
  category: string;
...
export interface CaseIndex {
  schema: string; // "atalaya.index/v1"
