...
\newcommand{\versiondoi}{10.5281/zenodo.22834339}
\newcommand{\conceptdoi}{10.5281/zenodo.21511294}
\newcommand{\docrepo}{github.com/fsantibanezleal/CAOS\_ATALAYA}
...
CAOS open-research programme \quad$\cdot$\quad CAOS\_ATALAYA, relation discovery over open-data catalogs\\[2pt]
...
\title{Atalaya: A Calibrated Multi-Evidence\\ Affinity Graph over an Open-Data Catalog,\\ with a Shuffled-Alignment Negative Control\\ for False Discoveries}
...
\thanks{Source code and derived artifacts are released under the MIT license at
\url{https://github.com/fsantibanezleal/CAOS_ATALAYA}; a live instance is available at
\url{https://atalaya.fasl-work.com}.}}

\markboth{\doctype\ \docversion, 2026 \textperiodcentered\ CAOS open-research programme}%
{Santib\'a\~nez-Leal: A Calibrated Multi-Evidence Affinity Graph over an Open-Data Catalog}
...
knowledge graph, and ranks pairwise relatedness with a \emph{calibrated multi-evidence affinity} that fuses
the evidence with standard empirical-CDF calibration and reliability weighting. The
...
and it down-weights any evidence that contradicts the others. Two validation measurements accompany the graph.
First, the correlation miner is tested with a \emph{shuffled-alignment negative control}: on the same alignments
with the keys shuffled, $343$ candidate correlations yield $0$ survivors under a permutation null with
Benjamini--Hochberg control (empirical false-discovery rate $0.00$), which gives empirical support to the $24$
correlations reported on real data. Second, the semantic encoder is compared with a classical lexical baseline,
over which its measured margin is modest ($+0.014$ top-$5$ neighbour-theme coherence over TF--IDF). Every edge
carries its evidence in the graph database, and the whole affinity is pure \texttt{numpy}, so the browser
recomputes it live as an analyst reweights the evidences. The graph reports where datasets can be related and how
strongly relative to chance; it does not assert causation.
\end{abstract}
...
column, which describe the same phenomenon under different names, and which carry indicators that move
...
the raw magnitude of a signal whose scale is arbitrary. And it must be \textbf{auditable}: every edge
should carry the evidence that justifies it, the statistical edges should survive a negative control, and a
learned component should be measured against a classical baseline, with the measured margin reported.
...
\item A \textbf{relation-mining pipeline} (Section~\ref{sec:evidence}) that produces five orthogonal
edge kinds over a real national catalog, each recording its own evidence, with a correlation miner built from a
seeded permutation null and Benjamini--Hochberg false-discovery control (a first-order partial-correlation
confounder check is implemented but is not applied in the run reported here).
\item A \textbf{validation with negative controls} (Section~\ref{sec:validation}) centred on a
\emph{shuffled-alignment negative control}: re-mining correlations on shuffled key alignments yields an
empirical false-discovery rate of $0.00$ ($0$ of $343$ candidate correlations survive), and the semantic
encoder is benchmarked against a TF--IDF lexical baseline, over which its gain is modest.
\item A \textbf{reproducible, explorable instance} over 1{,}017 datasets (Section~\ref{sec:results}), with all
derived artifacts archived in the repository and every figure in this paper regenerated deterministically from
them.
\end{enumerate}

Section~\ref{sec:scope} states the scope: the graph reports where datasets \emph{can} be related and how
strongly relative to chance; it does not assert causation, and its tunables are set for one government-Chile
catalog. The contribution is not a claim of discovery but a method for turning a flat catalog into an auditable
relational map in which every relation is stated together with the strength of its evidence.
...
government direct-file tier, downloads and profiles the underlying tables in full. In the snapshot used here
...
All later stages consume only these cached artifacts; no stage described here requires network access or a live
recomputation, so the derived graph, the evaluation metrics, and the figures are all reproducible from the
...
\item \textbf{SPATIALLY\_OVERLAPS}: both datasets are municipality\slash region-keyed or carry coordinates in the same
...
control~\cite{bh} across the whole candidate family. The most expensive edge to mine, and the one whose
interpretation demands the most care (see Section~\ref{sec:scope}).
\end{itemize}
...
global cap on the number of tests, both logged if hit) so the multiple-testing correction is taken over a fixed
denominator that counts every test performed. Every accepted edge additionally receives the fused affinity of Section~\ref{sec:affinity} as
...
all three evidences. The background samples (small sorted arrays) are fit offline and distributed with the graph so
...
and distributed background arrays, the browser recomputes $S$ for the visible subgraph in real time as the analyst
...
\section{Validation with Negative Controls}\label{sec:validation}

\begin{figure*}[!t]
\centering
\includegraphics[width=0.80\textwidth]{fig-validation.pdf}
\caption{Validation measurements (data: \texttt{metrics.json}). \textbf{(a)} Shuffled-alignment negative control:
...
empirical false-discovery rate of $0.00$. \textbf{(b)} Learned embedding versus a lexical baseline: the MiniLM
embedding reaches $0.944$ top-$5$ neighbour-theme coherence against $0.930$ for a TF--IDF lexical baseline over
the same text, both far above the theme base rate of $0.478$; the embedding gain is a modest $+0.014$.}
\label{fig:validation}
\end{figure*}

The relevant question about a relatedness graph is not whether links are found but whether they are stronger than
in a null world. We address it with four checks, negative controls and leakage-safe measurements, all computed
in the evaluation stage and stored in \texttt{metrics.json}.

\textbf{Shuffled-alignment negative control.} The correlation edges are the ones most easily produced spuriously
by multiple testing, so we subject them to a negative control. We take the exact indicator series and key
...
$q=0.05$. A miner whose false discoveries are controlled should find essentially nothing on shuffled data. It
finds nothing: of $343$ candidate pairs that clear the effect-size screen on the shuffled data, \textbf{$0$}
survive the null and FDR control, an empirical false-discovery rate of \textbf{$0.00$}
(Fig.~\ref{fig:validation}a). This measurement is the empirical support for the $24$ correlations reported on
real data; without it, false-discovery control would be an assumption of the procedure rather than a measured
property of the miner.

\textbf{Learned embedding versus a lexical baseline.} Rather than assume that a learned embedding outperforms a
lexical baseline, we measure the gap. Over the identical set of datasets we compute top-$5$
...
far above the theme base rate of $0.478$ (Fig.~\ref{fig:validation}b). The gain of the embedding is positive but
modest ($+0.014$), which justifies using the embedding as \emph{one} calibrated evidence rather than as the whole
ranking.
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.80\textwidth]{fig-evidence.pdf}
\caption{Edge-kind composition of the graph ($14{,}413$ edges over $1{,}017$ datasets), coloured by evidence
strength. The graph makes its evidence hierarchy explicit: a small number of \emph{hard} structural links
($117$ joins, $24$ FDR-passing correlations), more \emph{medium} semantic and spatial evidence, a large but
\emph{soft} same-source prior, and the $4{,}053$ fused, null-calibrated affinity summaries. The interface renders
the same hierarchy, so that a cheap prior is not presented to the analyst as a strong relation.}
\label{fig:evidence}
\end{figure*}

The graph over the $1{,}017$-dataset catalog has $14{,}413$ edges (Fig.~\ref{fig:evidence}):
$6{,}002$ SAME\_SOURCE, $3{,}948$ SEMANTICALLY\_SIMILAR, $269$ SPATIALLY\_OVERLAPS, $117$ JOINABLE\_ON, $24$
CORRELATES, and $4{,}053$ AFFINITY summary edges. The composition is informative in itself: the vast
majority of raw edges are cheap priors and topical similarities, while the structurally hard evidence (joins and
FDR-passing correlations) is a small, strictly filtered minority. The affinity score exists precisely so that this
...
salud vigentes} (educational and active health establishments) at $\rho=0.528$, permutation $p=0.001$
(accepted under BH-FDR at $q=0.05$), over $n=324$ municipalities aligned on \dsname{comuna\_cut}. The aligned
columns here are administrative region and health-authority codes, so this $\rho$ reflects a shared
north-to-south regional ordering rather than establishment co-location; we report it as a code-level
association, not a substantive relation, and not a cause. A representative join edge connects two municipality-keyed tables with MinHash
...
and it is tuned for one catalog. Six limitations apply.

\textbf{Association, not causation.} A CORRELATES edge means two columns move together across municipalities
after a permutation null and FDR control; the correlation run reported here is \emph{not} partial-correlation
screened (a first-order confounder check is implemented but not applied). It does not identify a mechanism, and
a confounder can still induce it. The interface labels these edges as associations.

\textbf{Administrative-code correlations.} The correlation miner selects any numeric-castable column, so a share
of the $24$ CORRELATES edges align administrative or geographic \emph{code} columns (region, service and entity
codes) rather than substantive indicators, some over as few as eight aligned units, and a few are near-tautological
code-versus-code alignments. These should be read as code-level alignments, not indicator relationships.
Screening out identifier and code columns before correlating, and applying the partial-correlation check, are the
direct remedies.
...
richer dependence estimators at the cost of live recomputation in the browser.
...
on the semantic and same-source signals only. The graph reflects this directly: such datasets carry no hard
structural edges.

\section{Reproducibility and Availability}\label{sec:repro}

Every number in this paper comes from artifacts archived in the repository: the graph counts
(\texttt{graph.json.counts}), the evaluation metrics (\texttt{metrics.json}), the harvest report
(\texttt{harvest\_report.json}), and the per-case top-evidence artifacts. The archived
\texttt{graph.json} is a top-4{,}000-by-weight web payload carrying edge endpoints and weights only;
the full per-edge evidence is held in the graph database, which is kept outside the repository. The three figures
are regenerated deterministically by a single script from those artifacts (the two data figures) and from a
hand-authored, theme-aware vector schematic (the method figure), with no network access. The pipeline is staged
and seeded, and it is checked by determinism and data-contract tests. The relatedness statistics and the affinity
are pure \texttt{numpy}/\texttt{math}, so the same code runs in the offline pipeline and live in the browser; the
graph is also queryable from an agent via an included MCP server. Source, artifacts, and a live instance are open under the MIT
...
evidences into a single calibrated score, and by testing that graph against negative controls rather than
judging it by plausibility. The calibration against empirical null distributions makes a score mean ``stronger
than chance''; the reliability weighting makes contradicting evidence count for less; the term-by-term reporting
makes every edge auditable; and the shuffled-alignment negative control ($0$ of $343$ shuffled correlations
survive) gives the statistical edges an empirical false-discovery estimate that an uncontrolled miner lacks. The
learned embedding improves on the lexical baseline by a modest margin ($+0.014$), which is why it enters the score
as one calibrated evidence rather than as the ranking itself. The method is not tied to this catalog; what
transfers is the recipe of fuse, calibrate against a null, weight by reliability, and test against a shuffled
negative control, for anyone building a relatedness graph over a large, heterogeneous collection of datasets.
...
seeded generator ($1{,}000$ permutations in the miner, $500$ in the negative control) with add-one smoothing so a
...
search at query time; the sorted sample is distributed with the graph so the calibration is identical offline and in
