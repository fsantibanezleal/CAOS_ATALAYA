...
than overselling it. Every edge carries its evidence in the graph database, the whole affinity is pure \texttt{numpy} so the browser
...
confounder check is implemented but is not applied to the committed run).
...
after a permutation null and FDR control; the committed correlation run is \emph{not} partial-correlation
...
code-versus-code alignments. These should be read as code-level alignments, not indicator relationships; screening
out identifier and code columns before correlating, and applying the partial-correlation check, is the clear next fix.
...
Every number in this paper comes from artifacts committed to the repository: the graph counts
(\texttt{graph.json.counts}), the evaluation metrics (\texttt{metrics.json}), the harvest report
(\texttt{harvest\_report.json}), and the per-case top-evidence artifacts. The committed
\texttt{graph.json} is a top-4{,}000-by-weight web payload carrying edge endpoints and weights only;
the full per-edge evidence lives in the out-of-git graph database. The three figures are regenerated deterministically
